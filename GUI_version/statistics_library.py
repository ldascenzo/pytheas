...
class Stats:
    def __init__(self, targets, decoys, match, isotope, FDR, FDR_line, Sp_cutoff,
        ...
        self.FDR_input = FDR
        self.FDR_line_values, self.Sp_cutoff = FDR_line, Sp_cutoff
    ...
    def graphs_plot(self):
        ...
        if not csv_files[1].empty:
            ...
            plt.subplot(2, 1, 1)
            stats.scatter_nts_vs_score(csv_files, 'Sp', 0, 0, lengths, percentile_option,
            ...
            if self.scatterbox_lengths_z:
                try:
                    plt.subplot(2, 1, 1)
                    stats.scatter_nts_z_vs_score(csv_files, 'Sp', 0, 0, lengths, self.Sp_cutoff,
                                                 self.MS1_offset_min, self.MS1_offset_max, targets_decoys)
                    plt.subplot(2, 1, 2)
                    stats.box_nts_z_vs_score(csv_files, 'Sp', 0, 0, lengths, self.Sp_cutoff,
                                             self.MS1_offset_min, self.MS1_offset_max, targets_decoys)
            ...
            if self.targets_without_decoys:
                stats.FDR_update(csv_files, lengths, self.Sp_cutoff, name_input, self.MS1_offset_min,
                                 self.MS1_offset_max, targets_decoys)
    ...
    def final_output(self):
        ...
        if self.FDR_input and not self.FDR_line_values:
            print("WARNING! Input FDR file was selected but no output lines value was specified")
