...
class Visualize:
    ...
    def df_for_dSp2(self):
        ...
        with open(self.score_file, 'r') as input_file:
            for line in input_file:
                if line[0].isdigit():
                    sp = line.split()
                    mz, rt, sequence, rank = sp[0], sp[1].split('=')[1], sp[11], int(sp[6].split('=')[1])
    ...
    def scored_info(self, input_file):
        ...
        for line in open(input_file, 'r'):
            ...
            if flag == 1 and line[0].isdigit():
                ...
                rank, offset = int(split[6].split('=')[1]), abs(int(split[3].split('p')[0]))
    ...
    def df_output_html(self):
                    ...
                    if float(score) >= self.Sp_cutoff and float(dSp) <= self.dSp_cutoff \
                        ...
                        if not flag_x:
                            ...
                            d['m/z'].append('<a href="#{}_{}_{}_{}">{}</a>'.format(mz_isotopologues_correction, rt,
                                                                                   score, seq, mz))
                            d['m/z_RT'].append("{}_{}".format(mz, rt))
                            ...
                            d['charge'].append(sp[10]), d['isotope'].append(sp[8]), d['molecule'].append(sp[15]),
        ...
        if self.MS1_offset:
            df = df.astype({"MS1_offset(ppm)": int})
            df = df[abs(df['MS1_offset(ppm)']) <= abs(int(self.MS1_offset))]
...
def compute_dSp2(df_match, df_filtered):
    ...
    output_df = output_df[['m/z', 'RT', 'MS1_offset(ppm)', 'length', "5'-end", 'sequence', "3'-end", 'charge',
                           'isotope', 'Rank', 'Score (Sp)', 'dSp', 'dSp2', 'n/L', 'sequence_mods', 'molecule']]
