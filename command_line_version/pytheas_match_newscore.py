...
***DESCRIPTION***
This program matches a theoretical RNA digest obtained by digest scripts vs experimental data in mgf format and return the results for all precursor ions matching + MS2 ions,
ordered via an arbitrary determined scoring function

Scoring function with 5 ion series: sumI(matched) / sumI(all) * n / L * ( 1 + beta[a + b] + beta[a-B] + beta[c + d] +
beta[w + x] + beta[y + z + y-P + z-P])

Scoring function with 11 ion series: sumI(matched) / sumI(all) * n / L * ( 1 + beta[a] + beta[a-B] + beta[b] + beta[c]
+ beta[d] + beta[w] + beta[x] + beta[y] + beta[z] + beta[y-P] + beta[z-P])

sum(I) = sum of the (normalized to the highest sequence-meaningful ion) intensities of matching MS2 ions, excluding non
sequence-meaningful ions (only  11 main series) within the specified m/z window
n = total number of matching MS2 ions for the given precursor, excluding non sequence-meaningful ions
(only 11 main series) within the specified m/z window
beta is determined as:
			...
			(alpha * consecutive_matches ) * beta_increment) over all consecutive matches in the same
			ion series (a,a-b,b..) or
			ion series group (e.g. a/b, w/x...). beta_increment and alpha are tunable via input options

L = normalization factor base on total number of theoretical predicted ions for a precursor ion excluding non-sequence
meaningful ions (only 11 main series), wihin the specified m/z window

***USAGE***
python pytheas_match_newscore.py --OPTIONS
...
***INPUT***
Digest file and .mgf file, both specified as option

***OUTPUT***
match_output_MGFNAME.txt - File with matches between measured peaks and the digest, all precursor ions in the mgf are
 listed, also if no matches are found
FDR_file -> FDR estimation on the matched/scored data, using only the target with at least one decoy for each
combination of M/RT
...
parser.add_argument('--digest_file', required=True, help='MS2 theoretical digest file (required)')
parser.add_argument('--mgf_file', required=True,
                    help='experimental measured peaks in .mgf format file for matching/scoring (required)')
parser.add_argument('--light_heavy', default='all', choices=['light', 'heavy', 'all'],
                    help='Specify if light, heavy or both species have to be considered for matching/scoring '
                         '(default = all)')

parser.add_argument('--MS1_mzlow', default=400, type=int,
                    help='Minimum m/z for MS1 ions to be considered for matching/scoring (Optional, default = 400)')
parser.add_argument('--MS1_mzhigh', default=2000, type=int,
                    help='Maximum m/z for MS1 ions to be considered for matching/scoring (Optional, default = 2000)')
parser.add_argument('--MS2_mzlow', default=300, type=int,
                    help='Minimum m/z for MS2 ions to be considered for matching/scoring (Optional, default = 300)')
parser.add_argument('--MS2_mzhigh', default=2000, type=int,
                    help='Maximum m/z for MS2 ions to be considered for matching/scoring (Optional, default = 2000)')
parser.add_argument('--MS1_ppm', default=30, type=int,
                    help='ppm window (+-) used for matching on MS1 ions (Optional, default = 30)')
parser.add_argument('--MS2_ppm', default=50, type=int,
                    help='ppm window (+-) used for matching on MS2 ions (Optional, default = 50)')
parser.add_argument('--MS1_ppm_offset', default=0, type=int,
                    help='Offset in ppm to "center" all the MS1 measured m/z in the matching process. '
                         'Can be a positive or negative number (Optional, default = 0)')
parser.add_argument('--MS2_ppm_offset', default=0, type=int,
                    help='Offset in ppm to "center" all the MS2 measured m/z in the matching process.'
                         ' Can be a positive or negative number (Optional, default = 0)')
parser.add_argument('--MS2_peak_int_min', default='all',
                    help='Minimum absolute intensity threshold for MS2 ions to be included in matching/scoring'
                         ' (Optional, default = None)')
parser.add_argument('--MS2_peak_num_max', default='all',
                    help='Number of most intense MS2 ions of the same precursor to be considered for matching/scoring'
...
parser.add_argument('--precursor_window_removal', default=2, type=float,
                    help='Value of +-Da around precursor ion (and precursor ion + 1/2NA if NA_removal option is '
                         'selected) to exclude for matching of MS2 ions (Optional, default = 2)')
parser.add_argument('--losses_window_removal', default=1.5, type=float,
                    help='Value of +-Da around M-xx matched losses and free bases to exclude when '
                         'calculating the contributions to sumI_all (Optional, default = 2)')
...
parser.add_argument('--beta_increment', default=0.075, type=float,
                    help='Value to be added to the scoring function as increment for each occurrence of '
                         'consecutive ions found in the same ion series (eg a1/a2) (Optional, default = 0.075)')
parser.add_argument('--alpha', default=0, type=float,
                    help='Value of the incremental step for each consecutive match in the same MS2 series '
                         '(Optional, default = 0)')
parser.add_argument('--MS2_normint_cutoff', default=5, type=int,
                    help='Cutoff value for MS2 ions NORMALIZED intensity (based on top match = 100). '
                         'Everything below the threshold is excluded for scoring purposes (Optional, default = None)')
parser.add_argument('--precursor_isotopologues', default='n', choices=['y', 'n'],
                    help='Choose (y/n) if the +-1 isotopologue peaks have to be considered for matching'
                         ' of precursor ions (Optional, default = n)')
parser.add_argument('--use_charges_mgf', default='y', choices=['y', 'n'],
                    help='Choose (y/n) if the charges in the mgf input file for precursor ions '
                         'will be used for matching (Optional, default = y)')
...
parser.add_argument('--only_targets_with_decoys', default='y', choices=['y', 'n'],
                    help='Select (y/n) if only targets with at least one decoy will be used for the FDR table '
                         '(Optional, default = y)')
...
def mod_detection(sequence):
    """
    Finds the list of modified bases present in the matching MS2 ions, to be used for sumI, n, L calculations
    """
    mod_bases = []
    for nt in sequence:
...
def ppm_offset(measured_mass, theoretical_mass):
    """
    Calculates the ppm offset between matching m/z values for MS1/MS2
    """
    difference = np.float64(measured_mass) - np.float64(theoretical_mass)
    ppm_offset = difference / np.float64(theoretical_mass) * 1000000

    return int(round(ppm_offset, 0))


def threshold_MS2_int(t=args.MS2_peak_int_min):
    """
    Defines minimum intensity threshold for MS2 ions matching (given as option)
    """
    if t == 'all':
        threshold = 0
    else:
...
def reduced_digest(digest_peaks):
    """
    Creates a reduced version of the digest with only the M-xx ions, to be used in the step of normalization of
    intensities
    """
    output_dic = {}
    ...
    for key in match_peaks:
        if match_peaks[key]:
            ...
            for t in match_peaks[key]:
                ...
                # Loops among all ions in the mgf ordered by intensity
                for f in mgf_peaks[key]:
                    ...
                    for peak in digest[t][-1]:
                        ...
                        if (mz_mgf < args.MS2_mzlow or mz_mgf > args.MS2_mzhigh or (mz_digest -
                                                                                    ppm_range(mz_digest,
...
def MS2_max_peaks(m=args.MS2_peak_num_max):
    """
    Returns the number of most intense MS2 ions to limit the search to (given as option)
    """
    if m == 'all':
        max_value = 999999
    else:
...
def consecutive_series_5(list_MS2, ion_series):
    """
    Returns beta for each of the 5 ion series groups.
    beta is determined as:
...
def L(mz_match, d, mod_bases):
    """
    Returns value L for the scoring function. Only ions that carry sequence information are considered (11 series)
    ...
    for ion in d[mz_match][-1]:
        ...
        if args.MS2_mzlow <= np.float64(ion.split(':')[1]) <= args.MS2_mzhigh:
            series = ion.split('(')[0]
...
def sumI(list_MS2, mod_bases):
    ...
    for l in list_MS2:
        if args.MS2_mzlow < np.float64(l[2]) < args.MS2_mzhigh:
...
def n_calc(list_MS2, mod_bases):
    """
    Returns the value n (number of MS2 ions) for the scoring function disregarding multiple charged species for the same series
    ...
    for l in list_MS2:
        ...
        if args.MS2_mzlow < np.float64(l[0]) < args.MS2_mzhigh:
...
def closest_value(mz_list, mz_precursor):
    """
    Returns the closest mz value to the precursor m/z within the MS2 list in the mgf file
...
def pick_max_intensity(ion_list):
    """
    Returns the MS2 ion to pick in case of multiple matching by the same theoretical ion (the most intense one)
    """
...
def digest_dic(digest_file=open(os.getcwd() + "/" + args.digest_file, 'r')):
    """
    Makes a dictionary with the info from the digest file given as input
...
def mgf_dic(mgf_file=open(os.getcwd() + "/" + args.mgf_file, 'r'), mz_freeb=mz_freebases()):
    """
    Makes a dictionary with MS1/MS2 ions from input .mgf file

    Format -> {m/z_precursor_RT : { m/z ion1 : norm_intensity1 , m/z ion2 : norm_intensity2 ...}, ... }
    """

    d = {}
    # Creates a dictionary with all precursor masses _ RT as keys, to be filled with info for the final output
    ...
    for line in mgf_file:
        ...
        # Sets values for prec_mass, rt and charge in order to avoid errors when those values are missing from the mgf
        ...
        # Determines the intensity for the precursor ion peak (closest value within MS2 scans to the PEPMASS)
        if "END IONS" in line:
            # Controls on charges and m/z boundaries for MS1 precursor ions
            if args.MS1_mzlow <= np.float64(prec_mass) <= args.MS1_mzhigh:
    ...
    for key in d:
        ...
        # Creates a list with all MS2 ions entries, if m/z of MS2 ions are within the specified MS2 m/z window
        for k in d[key]:
            if np.float64(k) >= args.MS2_mzlow and np.float64(k) <= args.MS2_mzhigh:
                lista.append([k, d[key][k]])

            # Adds MS2 ions of free bases even if they are outside the specified MS2 window    
            else:
                for ion in mz_freeb:
...
def matching(digest_dic=dig_dic, mgf_dic=mgf_dic()):
    """
    Matches the MS1/MS2 ions between theoretical digest and experimental .mgf file
    The 'all' addition are meant to create an additional dictionary with a list of all ions in the mgf irrespective of matching
    """
    ...
    for key in mgf_dic:
        ...
        if args.precursor_isotopologues == 'n':
            ...
            if args.use_charges_mgf == 'y':
                prec_match = (dict((k, v) for k, v in digest_dic.items() if prec_charge == int(k.split('_')[2])
                                ...
                                ppm_range(prec_mass, args.MS1_ppm_offset), args.MS1_ppm) <= np.float64(k.split('_')[0])
                                <= prec_mass + ppm_range(prec_mass, args.MS1_ppm_offset) + ppm_range(prec_mass +
                                ppm_range(prec_mass, args.MS1_ppm_offset), args.MS1_ppm)) or (prec_mass +
                                neutron_mass / abs(prec_charge) + ppm_range(prec_mass, args.MS1_ppm_offset) -
                                ppm_range(prec_mass + ppm_range(prec_mass, args.MS1_ppm_offset), args.MS1_ppm) <=
        ...
        for key_match in prec_match:
            ...
            for x in prec_match[key_match][-1]:
                ...
                # Selects only MS2 ions that have a relative intensity above a certain threshold
                if add:
            ...
            for a in map(list, mgf_dic[key].items()):
                if a[0] not in matched_ions.keys():
                    all_mgf_list.append([args.MS2_mzlow + flag, ''] + a)

                else:
                    all_mgf_list.append([args.MS2_mzlow + flag, matched_ions[a[0]]] + a)

                flag += 0.01

            # Adds the ions to the dictionary of MS2 ions
            if MS2_list:
    ...
    # Obtains a reduced version of the digest with only M-xx ions to be excluded in the normalization
    red_digest = reduced_digest(digest_dic)

    # Normalizes the intensities of matching MS2 ions
...
def scoring(d, d_all, dig=dig_dic):
    """
    Adds a score to the precursor ion matches
    """
    d2 = {}
    for key_rt in d:
...
def consolidate_match(dic=scoring(dic, dic_all)):
    ...
    !!!A control for the case of multiple precursor with the same RT has to be added at this point!!!
    !!!Also control on multiple matching of precurso ions within the given ppm window!!!
    """
    lista, d = [], {}
...
def output(match_dic=consolidate_match()):
    """
    Writes the lines for the output file
    """
    global MS1_hits, MS2_hits
    MS1_hits, MS2_hits, list_ion_rt = 0, 0, []

    output_lines = ["#in_silico_digest " + args.digest_file + "\n#enzyme " + str(
        enzyme) + "\n#mgf_file " + args.mgf_file + "\n#MS1_ppm " + str(args.MS1_ppm) + "\n#MS2_ppm " + str(
        args.MS2_ppm) + "\n#MS1_offset_ppm " + str(args.MS1_ppm_offset) + "\n#MS2_offset_ppm " + str(
        args.MS2_ppm_offset) + "\n#MS1_mzlow " + str(args.MS1_mzlow) + "\n#MS1_mzhigh " + str(
        args.MS1_mzhigh) + "\n#MS2_mzlow " + str(args.MS2_mzlow) +
                    "\n#MS2_mzhigh " + str(args.MS2_mzhigh) + "\n#MS2_peak_int_min " + str(
        ...
        args.all_series) + "\n"]
    output_lines.append(
        "#MATCHES_HEADER:m/z(meas) RT(mins) m/z(theo) offset intensity_prec abs_sumI dSp Sp Sp_one length #MS2_matches "
        "light/heavy charge seq mod 3'-end 5'-end MS2_matches-->m/z_measured(offset(ppm))"
        "[norm_intensity]:m/z_theoretical[CID_ion]\n\n")
    ...
    for key in sort:
        ...
        for m in list_prec_sorted:
            ...
            for l in m:
                ...
                line = (matched_mass + " RT=" + str(round(np.float64(l[1]) / 60, 3)) + " TH_MATCH=" + l[
                    2] + " " + MS1_ppm + "ppm " + "INT_PREC=" + info_MS2_scans[l[0] + "_" + l[1]][
                            0] + " ABS_sumI=" + str(round(np.float64(info_MS2_scans[l[0] + "_" + l[1]][1]) *
                                                          np.float64(l[3].split(';')[0].split('=')[-1]) / 100,
                                                          1)) + " " + str(round(delta_sp, 2)) + " SCORE=" + str(
                    l[3].split("(")[0]) + " " + str(score) + " " + str(len(l[6].split()[2])) + " #MS2=" + l[
                            5] + " " + " ".join(l[6:]) + " " + str(counter) + " SCORE=" + str(l[3]) + "\n")
...
def log_file():
    """
    Writes a log file with info about total MS1 matches and 'productive' matches (with at least one MS2 match)
    """
...
def csv_output(match_file):
    """
    Outputs an additional csv file that makes consultation and browsing of the output easier
    """
    csv_lines = []
    with open(match_file, 'r') as infile:
        for line in infile:
            if "PRECURSOR_ION" in line:
                csv_lines.extend(
                    ["\n", "Precursor m/z", "RT(min)", "MS1_offset", "", "Rank", "Score(Sp)", "dSp", "MS2 peaks matches #",
            ...
            if line[0].isdigit():
                ...
                csv_lines.extend([s[0], s[1].split('=')[1], s[3], "", s[-2], str(np.float64(s[8])), str(np.float64(s[6])),
...
if __name__ == "__main__":
    ...
    # Write an additional csv file to make visualization of the results easier
    open("./match_output_" + mgf_name + ".csv", 'w').writelines(csv_output(
        os.getcwd() + "/match_output_" + mgf_name + ".txt"))

    # Output specific for the statistical analysis
    stats.csv_output(
        stats.filter_data(stats.input_data("match_output_" + mgf_name + ".txt", 'n', 'y'), 'y', 'Sp'),
        ['all'], 'n', 0, mgf_name, args.only_targets_with_decoys,
        args.FDR_light_heavy)

    print("\nDone! Output file(s) -> {} {} {} {} {} {}".format("match_output_" + mgf_name + ".txt",
                                                                  "match_output_" + mgf_name + ".csv",
                                                                  "log.txt",
