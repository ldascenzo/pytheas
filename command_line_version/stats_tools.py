...
def input_data(infile, excel, sumi_all):
    ...
    if sumi_all == 'n':
        d = {'M': [], 'RT': [], 'M_RT': [], 'Theo_mono_M': [], 'isotopologue': [], 'nts': [], 'Sp': [], 'dSp': [],
             'n': [], 'sumI': [], 'L': [], 'n/L': [], 'Beta': [], 'charge': [], 'isotope': [], 'molecule': [],
             'sequence': [], 'sequence_ext': [],
             'MS1_ppm': [], 'MS2_ppm': [], 'MS2_matches': []}

    else:
        d = {'M': [], 'RT': [], 'M_RT': [], 'Theo_mono_M': [], 'isotopologue': [], 'nts': [], 'Sp': [], 'dSp': [],
             'n': [], 'sumI': [], 'sumI_all': [], 'L': [], 'n/L': [], 'Beta': [], 'charge': [], 'isotope': [],
             'molecule': [], 'sequence': [],
    ...
    with open("./" + infile, 'rU') as input_file:
        for line in input_file:
            ...
            if line[0].isdigit():
                ...
                d['M'].append(prec_M), d['nts'].append(int(sp[9])), d['Sp'].append(np.float64(sp[8])), d['dSp'].append(
                    sp[6]), d['charge'].append(sp[12]),
                d['isotope'].append(sp[11]), d['molecule'].append(sp[17]), d['sequence'].append(sp[13]), d[
                    'sequence_ext'].append(sp[14]), d['RT'].append(sp[1].split('=')[1]),
                d['M_RT'].append(prec_M + '_' + sp[1].split('=')[1]), d['MS1_ppm'].append(sp[3][:-3]), d[
                    'MS2_matches'].append(",".join(sp[18:-1])), d['Theo_mono_M'].append(
                    round(np.float64(sp[2].split('=')[1]), 5))
...
def filter_data(df, top, yl):
    ...
    if top == 'y':
        ...
        filt_df = filt_df[
            ['M', 'RT', 'M_RT', 'Theo_mono_M', 'isotopologue', 'nts', 'Sp', 'dSp', 'dSp_2nd_target', 'MS1_ppm', 'n',
             'sumI', 'sumI_all', 'L', 'n/L', 'Beta', 'charge', 'isotope', 'molecule', 'sequence', 'sequence_ext',
             'nts_z', 'MS2_ppm', 'MS2_matches']]
    ...
    if not np.isnan(decoy_max):
        ...
        for i in np.arange(0, round(decoy_max, 3) + step, step):
            ...
            if tot > 0:
                ...
                top_decoy = len(
                    merged_df[(merged_df['molecule'].str.contains('decoy') == True) & (merged_df['Sp'] > i)]['M'])
...
# def violinplot_Sp_vs_nts(df, y, y_bot, y_top, lengths, Sp_cutoff):
#     """
#     Violin plot
#     """
#     inplot_df, indecoy_df = df     
...
#     plot_df = inplot_df.loc[inplot_df['Sp'] > Sp_cutoff]
#         
#     # The Sp cutoff is applied only to targets, decoys that share the same mz_RT with targets above cutoff are always kept
#     decoy_df = indecoy_df[indecoy_df.M_RT.isin(plot_df.M_RT)]
#
#     max_nts = plot_df['nts'].max()
#     
...
#         
#         #target = plt.violinplot(data1, positions = [np.float64(x) + 0.15 for x in lenghts_labels])         
#         #decoys = plt.violinplot(data2, positions = [np.float64(x) - 0.15 for x in lenghts_labels])
#         target = plt.violinplot([val or nans for val in data1], positions = [np.float64(x) + 0.15 for x in lenghts_labels])
#         
##         for element in ['boxes', 'whiskers', 'fliers', 'means', 'medians', 'caps']:
##             plt.setp(target[element], color = 'blue')
##             plt.setp(decoys[element], color = 'red')
...
#     else:
#         lengths.sort()
#         target = plt.boxplot(data1, labels = lengths, positions = [np.float64(x) + 0.15 for x in lengths], manage_ticks = False, widths = 0.25, sym = 'b.')   
#         decoys = plt.boxplot(data2, labels = lengths, positions = [np.float64(x) - 0.15 for x in lengths], manage_ticks = False, widths = 0.25, sym = 'r.')   
#
#         for element in ['boxes', 'whiskers', 'fliers', 'means', 'medians', 'caps']:
#             plt.setp(target[element], color = 'blue')
#             plt.setp(decoys[element], color = 'red')
