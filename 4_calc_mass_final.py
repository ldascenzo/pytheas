...
def lines_final(input_lines, mzlow, mzhigh, nts_light=nts_mass(read_excel_input(args.nts_alphabet_light))[0]):
    ...
    for line in input_lines:
        ...
        elif line.split()[7] == "cP" or line.split()[7] == "CP":
            l_mass, h_mass = l_mass - ele_mass["O"] - ele_mass["H"], h_mass - ele_mass["O"] - ele_mass["H"]
...
if os.path.exists(os.getcwd() + "/output.3.MS2"):  # Checking if the input file output.3.MS2 is present
    ...
    def fragment_MS2_masses(line, MS2_charge_dic, nts_dic_heavy,
        ...
        if args.ion_mode == '-':
            ...
            # Add a proton in negative mode when a charge is lost due to a charged loss
            charge_correction = ele_mass['H']
        else:
            ...
            # Subtract a proton in positive mode when a positive charge is lost due to charged loss
            charge_correction = - ele_mass['H']
        ...
        if chem3prime == 'P':
            # Adds the neutral phosphate loss ions M-PO3H and M-PO4H3 to the MS2 list if 3' end is P
            ...
            if charge_prec > 1:
                MPO3_mz = (mass_prec * charge_prec + dic_M_x_series['M-PO3'] + ele_mass[
                    "H"]) / charged_loss
                ...
                MPO4H2_mz = (mass_prec * charge_prec + dic_M_x_series['M-PO4H2'] + ele_mass[
                    "H"]) / charged_loss
        ...
        elif chem3prime == 'cP' or chem3prime == 'CP':
            ...
            if line.split()[1] == "light":
                ...
                if charge_prec > 1:
                    ...
                    if args.MS2_mzhigh >= MB_mz >= args.MS2_mzlow:
                        output_string = 'M-{}({}):{}'.format(b, line.split()[5][0] + str(charged_loss), str(round_masses(MB_mz)))
                        if output_string not in output_masses:
                            output_masses.append(output_string)
                    ...
                    if chem3prime == 'P':
                        ...
                        MBB_mz = (np.float64(line.split()[0]) * charge_prec + charge_correction - nts_dic_onlyB_light[
                            b] - nts_dic_onlyB_light[last_nt]) / charged_loss

                        # If m/z is in the given interval for MS2, result is printed
                        if args.MS2_mzhigh >= MBB_mz >= args.MS2_mzlow:
                            output_string = 'M-{}-{}({}):{}'.format(b, last_nt, line.split()[5][0] + str(charged_loss),
                ...
                if chem3prime == 'P':
                    ...
                    MBHP_mz = np.float64(line.split()[0]) - (
                            nts_dic_onlyB_light[b] + charge_correction - dic_M_x_series['M-PO3H']) / charge_prec
                    ...
                    # Add the special case of base double loss (with one being the 3'end base) for CP 3' chemistry
                    MBHBH_mz = np.float64(line.split()[0]) - (nts_dic_onlyB_light[b] + 2 * charge_correction +
                ...
                if charge_prec > 1:
                    ...
                    if chem3prime == 'P':
                        ...
                        # Add the special case of base double loss (with one being the 3'end base) for CP 3' chemistry
                        MBBh_mz = (np.float64(line.split()[0]) * charge_prec + charge_correction - nts_dic_onlyB_heavy[
                            b] - nts_dic_onlyB_heavy[last_nt]) / charged_loss
                ...
                if chem3prime == 'P':
                    # Add the neutral simultaneous loss of both P and B
                    MBHPh_mz = np.float64(line.split()[0]) - (nts_dic_onlyB_heavy[b] + charge_correction -
                    ...
                    # Add the special case of base double loss (with one being the 3'end base) for CP 3' chemistry
                    MBHBHh_mz = np.float64(line.split()[0]) - (nts_dic_onlyB_heavy[b] + 2 * charge_correction +
                                                               nts_dic_onlyB_heavy[last_nt]) / charge_prec

                    # If m/z is in the given interval for MS2, result is printed
                    if args.MS2_mzhigh >= MBHBHh_mz >= args.MS2_mzlow:
                        output_string = 'M-{}-{}({}):{}'.format(b, last_nt, line.split()[5], str(round_masses(MBHBHh_mz)))
                        if output_string not in output_masses:
                            output_masses.append(output_string)

        # Cycles through all the CID series to output the MS2 ion masses
        for ion in args.CID_series:
    ...
    def header_info_MS2(input_file):
        ...
        header_lines.append("#CID_SERIES " + " ".join(args.CID_series) + "\n")
    ...
    """
    open(os.getcwd() + "/Digest_MS2.txt", 'w').writelines(
        header_info_MS2(open(os.getcwd() + "/output.3.MS2", 'r')) + final_lines_MS2(lines_MS2))
