...
***INPUT****
Standard input file is output.3.MS2, generated by running 3_consolidate.py in the same directory.
Note that if outfile.3 is not present the script will stop with an error
Dictionary file nts_light.csv, generated by 2_modify.py, with all nucleotides (+ modifications) and their IDs.
Note that if nts_light.csv is not present the script will stop with an error

***OUTPUT***
output.3.MS2 - file obtained after modification of the theoretical digest + decoys
